...
f = glob.glob('/home/divya/Desktop/NTIRE18_dataset/NTIRE2018_Train_Clean/*.jpg')
f.sort()
fs = glob.glob('/home/divya/Desktop/NTIRE18_dataset/NTIRE2018_Train_Spectral/*.npy')
fs.sort()
batch_size=4
...
for i in range(epoch_num):
    for j in range(iter_num):
        input_img = smisc.imread(f[j])
        input_img = np.asarray(input_img)
        train_input=np.zeros((4,3,256,256))
        train_input[0,0,:,:] = input_img[0:256,0:256,0]
        train_input[0,1,:,:] = input_img[0:256,0:256,1]
        train_input[0,2,:,:] = input_img[0:256,0:256,2]
        train_input[1,0,:,:] = input_img[256:512,256:512,0]
        train_input[1,1,:,:] = input_img[256:512,256:512,1]
        train_input[1,2,:,:] = input_img[256:512,256:512,2]
        train_input[2,0,:,:] = input_img[0:256,256:512,0]
        train_input[2,1,:,:] = input_img[0:256,256:512,1]
        train_input[2,2,:,:] = input_img[0:256,256:512,2]
        train_input[3,0,:,:] = input_img[256:512,0:256,0]
        train_input[3,1,:,:] = input_img[256:512,0:256,1]
        train_input[3,2,:,:] = input_img[256:512,0:256,2]
        input_spec = np.load(fs[j])
        train_spec=np.zeros((4,31,256,256))
        for chan in range(31): 
            train_spec[0,chan,:,:]=np.transpose(input_spec[:256,:256,chan])
            train_spec[1,chan,:,:]=np.transpose(input_spec[256:512,256:512,chan])
            train_spec[2,chan,:,:]=np.transpose(input_spec[0:256,256:512,chan])
            train_spec[3,chan,:,:]=np.transpose(input_spec[256:512,0:256,chan])
        train_input=train_input/np.max(train_input)
        train_spec=train_spec/np.max(train_spec)
        train_input = torch.from_numpy(train_input)
        train_spec = torch.from_numpy(train_spec)
        train_input = train_input.cuda()
        train_spec = train_spec.cuda()
        out = net(train_input)
        optimizer.zero_grad()
        total_loss = mse(train_spec,out)
        total_loss.backward()
        optimizer.step()
        a = torch.zeros(4,31,256,256)
        a = a.type(dtype)
        nsr = mse(train_spec,out)/mse(train_spec,a)
        print("Epoch num ",i," iteration num ",j,": nsr is ",nsr)

torch.save(net,'UNet_NTIRE.pth')